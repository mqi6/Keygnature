...
class Metric:
    ...
    def eer_compute(scores_g, scores_i):
        far, frr, ths = [], [], []
        ini = torch.min(torch.cat([scores_g, scores_i])).item()
        fin = torch.max(torch.cat([scores_g, scores_i])).item()
        paso = (fin - ini) / 10000
        thr = ini - paso
        while thr < fin + paso:
        ...
        idx = torch.argmin(gap).item()
        return (far[idx] + frr[idx]) / 2 * 100, ths[idx]
    ...
    def cal_user_eer_aalto(feats, num_enroll, num_verify):
        ...
        for i in range(U):
            ...
            accs.append(eer)
            ths.append(th)
        return sum(accs)/len(accs), sum(ths)/len(ths)
